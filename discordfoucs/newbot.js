// pomo-muter-bot.js — discord.js v14 (TIMER-BASED)
// Bot #2: runs its own Pomodoro wall-clock schedule (Asia/Riyadh).
// 00 -> focus-start  => Server Mute (all connected)
// 25 -> break-start  => Unmute (all connected)
// 29 -> break-1-left => optional cleanup hook (log only)
// 20 -> focus-5-left => log only
// Includes optional live monitor: auto-mute anyone joining during focus.
...
const TOKEN   = process.env.BOT_TOKEN1; // second bot token via env
const GUILD_ID = '1399596680822915124';

// ---- rate settings for queue ----
const TICK_MS      = 1000;  // process a batch every 1s
const MAX_PER_TICK = 6;     // 6 ops/sec
const SLEEP_429_MS = 3000;  // backoff on 429
...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildVoiceStates, // who is in voice/stage
    GatewayIntentBits.GuildMembers,     // fallback fetch for members (optional but useful)
  ],
...
// ---- queue ----
let queue = [];   // [{ userId, channelId, kind: 'mute'|'unmute' }]
let tickHandle = null;
let pausedUntil = 0;

// ---- current mode for live-monitor ----
let currentMode = 'idle'; // 'focus' | 'break' | 'idle'
...
// event minutes inside the 30-min block
function eventAtMinute(minute) {
  const mod = minute % 30;
  if (mod === 20) return 'focus-5-left'; // log only
  if (mod === 25) return 'break-start';  // UNMUTE
  if (mod === 29) return 'break-1-left'; // log only / cleanup hook
  if (mod === 0)  return 'focus-start';  // MUTE
  return null;
}
...
// ---- live monitor: auto-mute joiners during focus ----
client.on('voiceStateUpdate', (oldState, newState) => {
  try {
    ...
    if (!oldState.channelId && newState.channelId && !newState.serverMute) {
      ...
      // ticker will process it
    }
  } catch {}
});

// ---- main tick (timer-based) ----
const fired = new Set(); // `${dateKey}-${H}-${m}-${event}`

async function clockTick() {
  const t = nowParts();

  // keep currentMode in sync (not strictly required but informative)
  ...
  if (ev) {
    ...
    if (!fired.has(key)) {
      ...
      if (ev === 'focus-start') {
        ...
        await mass('unmute');
      } else if (ev === 'focus-5-left') {
        log(`tick: ${t.nowLabel} -> focus-5-left`);
      } else if (ev === 'break-1-left') {
        log(`tick: ${t.nowLabel} -> break-1-left`);
      }
    }
  }

  // daily cleanup of fired keys
...
client.once('ready', async () => {
  ...
  // kick an immediate tick, then every 10s
  await clockTick();
